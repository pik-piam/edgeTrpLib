...
for (reg0 in params$reg) {
  ...
  plotdata <- plotdata[!grepl("Eq inconvenience cost",variable)]
  plotdata <- plotdata[region==reg0][,region:=NULL]
  ...
  plotdata[, variable := factor(variable, levels = c("tot_VOT_price","fuel_price_pkm","non_fuel_price"))]
  
  p2 <- mipBarYearData(plotdata[period %in% params$yearsBarPlot],ylab="Prices logit VS1 without eq inconvenience costs [USD2005/pkm]", title = paste0("Prices logit VS1 ",reg0))
  p2 <- p2 + theme_mip(size = 18)
  print(p2)}
...
for (var0 in vars) {
  ...
  plotdata <- plotdata[!grepl("Eq inconvenience cost",variable)]
  plotdata <- plotdata[region %in% params$reg]
  plotdata <- droplevels(plotdata, data$regions %in% params$reg)
  plotdata <- plotdata[, variable := gsub("Logit cost\\|VS1\\|LDV\\|", "",variable)][period %in% params$yearsBarPlot]
  
  plotdata <- plotdata[, variable := gsub("Mini\\||Large\\||SUV\\||Van\\||Small\\||Medium\\|", "", variable)]
  plotdata[, variable := factor(variable, levels = c("tot_VOT_price","fuel_price_pkm","non_fuel_price"))]
  if (nrow(plotdata)>1){
    p2 <- mipBarYearDataMod(plotdata[period %in% params$yearsBarPlot],ylab = "Prices logit VS1 without eq inconvenience costs [USD2005/pkm]", title = paste0("Prices logit VS1 ",var0))
    p2 <- p2 + theme_mip(size = 18) 
    print(p2)}}
...
```{r Average LDV non fuel prices}
  ...
  plotdata <- data[grepl("Logit cost\\|S1S2\\|trn_pass_road_LDV_4W\\|non_fuel_price",variable) & period %in% params$yearsScen & region %in% params$reg]
  plotdata2 <- data[grepl("Logit cost\\|FV\\|(Liquids|BEV)\\|non_fuel_price",variable) & period %in% params$yearsScen & region %in% params$reg]
  plotdata <- rbind(plotdata,plotdata2)
  plotdata <- plotdata[, variable := gsub("Logit cost\\|S1S2\\||Logit cost\\|FV\\|", "",variable)]
  plotdata[, variable := factor(variable, levels = c("trn_pass_road_LDV_4W|non_fuel_price","BEV|non_fuel_price","Liquids|non_fuel_price"))]
...
plotdata <- data[grepl("Logit cost\\|FV\\|(Compact Car|Large Car|Large Car and SUV|Light Truck and SUV|Midsize Car|Mini Car|Subcompact Car|Van)\\|(Liquids|BEV)\\|non_fuel_price",variable) & period %in% params$yearsScen & region %in% params$reg]
plotdata[, variable := gsub("|Logit cost\\|FV\\||\\|non_fuel_price", "",variable)]
...
```{r Average LDV total price}
  ...
  plotdata <- data[grepl("Logit cost\\|S1S2\\|trn_pass_road_LDV_4W\\|",variable) & period %in% params$yearsScen & region %in% params$reg]
